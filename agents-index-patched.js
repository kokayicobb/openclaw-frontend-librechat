...
// OpenClaw proxy abort config
const OPENCLAW_PROXY_URL = process.env.OPENCLAW_PROXY_URL || 'http://host.docker.internal:18793';
const OPENCLAW_API_KEY = process.env.OPENCLAW_API_KEY || '';

// Map custom endpoint names to their session key prefixes (must match librechat.yaml headers)
const SESSION_KEY_MAP = {
  "'suelo": (convId) => `librechat:${convId}`,
  devin: (convId) => `agent:developer:librechat-dev:${convId}`,
};

/**
 * Fire-and-forget abort call to OpenClaw proxy.
 * @param {string} sessionKey - The OpenClaw session key
 */
function abortOpenClawSession(sessionKey) {
  fetch(`${OPENCLAW_PROXY_URL}/v1/chat/abort`, {
    method: 'POST',
    headers: {
    ...
    .then((r) => r.json())
    .then((result) => {
      logger.debug(`[AgentStream] OpenClaw abort result for ${sessionKey}:`, result);
    })
    .catch((err) => {
      logger.warn(`[AgentStream] OpenClaw abort failed for ${sessionKey}: ${err.message}`);
    });
}
...
router.post('/chat/abort', async (req, res) => {
  ...
  if (job && jobStreamId) {
    ...
    // Fire-and-forget: also abort the OpenClaw agent run via proxy
    const endpoint = abortResult.jobData?.endpoint;
    const sessionKeyFn = endpoint && SESSION_KEY_MAP[endpoint];
    if (sessionKeyFn) {
      const sessionKey = sessionKeyFn(jobStreamId);
      logger.debug(`[AgentStream] Sending OpenClaw abort for session: ${sessionKey}`);
      abortOpenClawSession(sessionKey);
    }
